...
public class PostimgDAO {
	...
	public ArrayList<PostimgVO> getAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				list.add(new PostimgVO(pno, ptitle, pwriter, pcontent, pdraw, pfile, plike, phit, pdate, pthumbnail));
	...
	public ArrayList<PostimgVO> getsearch(String name,String type) {
		ArrayList<PostimgVO> list = new ArrayList<>();
		sb.setLength(0);
		if(type.equals("p_title"))
			sb.append("select * from postimg where p_title like '%' || ? || '%' order by p_no desc");
		else if(type.equals("p_writer"))
			sb.append("select * from postimg where p_writer like '%' || ? || '%' order by p_no desc");
	...
	public void updateOne(PostimgVO vo) {
		...
		try {
			...
			pstmt.setString(4, vo.getPthumbnail());
	...
	public ArrayList<PostimgVO> getPageData(int pagenum)
	{
		ArrayList<PostimgVO> list = new ArrayList<>();
		
		sb.setLength(0);
		sb.append(" select * " + 
				  " from ( select  b.* , rownum rn from (select * from PostImg order by p_no desc) b ) c where rn between ? and ? ");
		...
		try
		{
			...
			pstmt.setInt( 1 , (pagenum*12)-11);
			pstmt.setInt( 2 , pagenum * 12);
			...
			while(rs.next())
			{
				list.add( new PostimgVO(
						rs.getInt("p_no")
						,rs.getString("p_title")
						,rs.getString("p_writer")
